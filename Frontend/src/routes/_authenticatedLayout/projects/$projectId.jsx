        ...
        <div className="md:w-1/2 p-4 md:sticky top-4 h-full">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            {project.thumbnailUrls.map((url, index) => (
              <div className='min-h-40 rounded-lg shadow-md ' key={index}>
                <Image src={url} alt={`Project Screenshot ${index + 1}`} className="rounded-md shadow-md h-full" />
              </div>
            ))}
          </div>
        </div>
...
export const Route = createFileRoute('/_authenticatedLayout/projects/$projectId')({
