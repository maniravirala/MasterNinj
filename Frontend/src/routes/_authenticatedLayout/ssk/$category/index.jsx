...
import Sidebar from "@/components/SSK/SskSidebar";
...
const SskCategory = () => {
  ...
  const handleCategoryChange = (category) => {
    navigate({ to: `/ssk/${TextToUrl(category)}` });
  ...
  const handleItemClick = (item) => {
    navigate({ to: `/ssk/${category}/${item.id}` });
...
export const Route = createFileRoute('/_authenticatedLayout/ssk/$category/')({
