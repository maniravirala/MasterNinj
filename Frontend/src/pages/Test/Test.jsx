...
const Test = () => {
  ...
  const handleItemClick = (item) => {
    ...
    const readmeText = item.details.readme || "No description available.";
    setReadme(readmeText);
  };

  const handleCategoryChange = (category) => {
    setSelectedCategory(category);
    setEssentials(category.replace(/\s+/g, ""));
    setShowCategory(false);
  };
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="mt-4 grid flex-1 grid-cols-1 gap-6 p-4 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3">
            ...
            {essentialsList[essentials].map((item, index) => (
              <Card
                key={index}
              ...
              >
                ...
                <Image src={item.imgURL} alt={item.name} className="h-48 w-full object-cover rounded-md" />
            ...
            {essentialsList[essentials].length === 0 && (
